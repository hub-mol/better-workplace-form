...
const PERSONAL_EMAIL_DOMAINS = [
  ...
  'icloud.com', 'yahoo.com', 'hotmail.com', 'outlook.com', 'protonmail.com',
];
...
function validateField(name, value) {
  ...
  if (name === 'phone') {
    ...
    if (cc?.prefix === '+48') {
      if (allDigits.slice(cc.ccLen).length !== 9)
...
function StepIndicator({ current }) {
  return html`
    <div class="form-stepper"
      role="progressbar" aria-valuenow=${current} aria-valuemin="1" aria-valuemax="3">
      ${STEPS.map(step => html`
        <div key=${step.id}
          class=${'form-stepper_pill'
            + (step.id < current ? ' is-done' : step.id === current ? ' is-active' : '')}
        ></div>
      `)}
    </div>
  `;
  ...
  return html`
    <div class="form_field-wrapper">
      <label for=${id} class="form_field-label">
        ${required && html`<span class="form_required" aria-hidden="true">*</span>`}
        ${label}
      </label>
      ${children}
...
function Step1({ data, errors, onChange, onBlur }) {
  const input = (name, type, autocomplete, placeholder) => html`
    <input class=${'form_input w-input' + (errors[name] ? ' is-validation-error' : '')}
  ...
  return html`
    <fieldset class="flex-col gap-xs form-step">
      ...
      <div class="form_field-wrapper">
        ...
        <textarea id="f_message" name="f_message" maxlength="5000" rows="7"
          placeholder="Np. interesują nas owoce i kawa dla 50 osób w biurze w Warszawie."
          class="form_input is-text-area w-input"
          onInput=${e => onChange('f_message', e.target.value)}>${data.f_message}</textarea>
      </div>
      <label class="w-checkbox form_checkbox">
        <div class=${'w-checkbox-input w-checkbox-input--inputType-custom form_checkbox-icon'
          + (data.agreemrk ? ' w--redirected-checked' : '')}></div>
        <input type="checkbox" id="agreemrk" name="agreemrk"
        ...
        <span class="form_checkbox-label w-form-label" for="agreemrk">
          Chcę otrzymywać od Betterworkplace Sp. z o.o. newslettera
          o tematyce benefitów pozapłacowych
        </span>
      </label>
      <p class="form_checkbox-label text-size-xs">
        Wysyłając ten formularz, wyrażasz zgodę na przetwarzanie Twoich danych
        przez Betterworkplace Sp. z o.o. i kontakt z Tobą w celu realizacji Twojego
        zapytania. Aby dowiedzieć się więcej o tym, jak dbamy o ochronę i poszanowanie
        Twojej prywatności, zapoznaj się z naszą${' '}
        <a href="https://www.betterworkplace.pl/privacy-policy"
          class="text-style-link-sm">Polityką prywatności</a>.
      </p>
    </fieldset>
  `;
}
...
function App() {
  ...
  const [data, setData]     = useState({
    ...
    tax_number: '', company_name: '', city: '',
    company_workers: '', department: '',
    f_message: '', agreemrk: false,
    url: '', brand: '',
  });
  ...
  useEffect(() => {
    ...
    const obs = new ResizeObserver(() => {
      window.parent.postMessage(
        { type: 'bwp:resize', height: document.documentElement.scrollHeight },
        '*'
      );
    });
  ...
  const handleNipLookup = useCallback(async () => {
    ...
    if (err) { setErrors(e => ({ ...e, tax_number: err })); return; }
    setNipLoading(true);
    setNipError('');
    try {
  ...
  const handleSubmit = useCallback(async (e) => {
    e.preventDefault();
    if (data.website) { setStatus('success'); return; } // honeypot
    ...
    return html`
      <div class="padding-xs">
        <div class="form_message-success w-form-done">
          <div data-wf--better-workplace--form-success-error-message--form-type="zapytanie"
            class="better-workplace--form_message">
            ...
            <div class="better-workplace--form_message_text">
              <p class="better-workplace--heading-style-h5">
                Dziękujemy!<br/>Twoje zapytanie zostało wysłane.
              </p>
              <p class="better-workplace--text-size-md">
                Nasz konsultant skontaktuje się z Tobą w ciągu 24h (dni robocze),
                aby omówić szczegóły dostępnej oferty.
              </p>
            </div>
          </div>
        </div>
  ...
  return html`
    ...
    <div id="form-component" class="form_component w-form">
      ...
      <form id="zapytanie" name="wf-form-zapytanie" data-name="zapytanie"
        method="post" novalidate class="flex-col gap-md"
        onSubmit=${handleSubmit}>

        ${step === 1 && html`<${Step1} data=${data} errors=${errors} onChange=${onChange} onBlur=${onBlur}/>`}
        ${step === 2 && html`
          <${Step2} data=${data} errors=${errors} onChange=${onChange} onBlur=${onBlur}
            onNipLookup=${handleNipLookup} nipLoading=${nipLoading}
            nipError=${nipError} nipFilled=${nipFilled}/>
        `}
        ...
        <div class="form-nav">
          <div class="form-nav_left">
            ${step > 1 && html`
              <button type="button" class="button is-secondary" onClick=${goBack}>Wstecz</button>
            `}
          </div>
          <div class="form-nav_right">
            ${step < 3 && html`
              <button type="button"
                class=${'button' + (currentStepHasErrors ? ' is-inactive' : '')}
                onClick=${goNext}>Dalej</button>
            `}
            ${step === 3 && html`
              <button type="submit"
        ...
        <input class="form-helper" name="website" tabindex="-1" autocomplete="new-password"
          value=${data.website ?? ''}
